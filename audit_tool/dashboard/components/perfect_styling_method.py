...
def create_metric_card(value: str, label: str, status: str = "default", description: str = None, status_text: str = None):
    """
    Creates a robust, standardized metric card that correctly renders HTML and supports optional descriptions and status text.
    ...
    status_text_html = ""
    if status_text:
        status_text_class = f"metric-status-{status}"
        status_text_html = f'<div class="metric-status {status_text_class}">{status_text.upper()}</div>'
